module V1
  class ProductsController < ApplicationController
    ...
    # PATCH /api/v1/products/?product_sku=75275105&inventory_center_id=2&quantity=2
    def update
      if product_params
    ...
    def set_inventory_center
      if params[:inventory_center_id]
        if InventoryCenter.where(id: params[:inventory_center_id]).any?
          ...
          render json: { success: false, inventory_center_id: @inventory_center, response: 'Inventory Center does NOT exists.' },
    ...
    def json_notfound_response(_response, message: 'Inventory center id not found', success: false,
                               status: :unprocessable_entity)
      render json: {
        success: success,
